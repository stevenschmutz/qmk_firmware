...
// Shift-the-next-letter state for DOT_SPC_OSFT. A real (not one-shot) shift
// is used because sm_td resolves mod-tap keys via tap_code16(), which sends
// a raw HID report using whatever *real* mods are currently registered and
// never consults get_oneshot_mods() - so a one-shot shift is silently
// dropped whenever the very next letter is an sm_td mod-tap key (i.e. almost
// always on this layout).
